...
public interface PostRepository extends JpaRepository<Post, Integer> {
  ...
  Page<Post> findAllByActiveTrueAndModerationStatusAndTimeBefore(ModerationStatus moderationStatus,
  ...
  int countAllByActiveTrueAndModerationStatusAndTimeBefore(ModerationStatus moderationStatus,
      Date time);

  Page<Post> findAllByTitleContainingOrTextContainingAndModerationStatusAndTimeBeforeAndActiveTrue(
      String title, String text, ModerationStatus moderationStatus, Date time, Pageable pageable);


  int countAllByTitleContainingOrTextContainingAndModerationStatusAndTimeBeforeAndActiveTrue(
      String title, String text, ModerationStatus moderationStatus, Date time);
  ...
  Page<Post> findAllByActiveTrueAndUserAndModerationStatus(User user,
      ModerationStatus moderationStatus, Pageable pageable);

  @Query(
      "select new main.api.general.calendar.CalendarObject(function('date',p.time), count(p)) from Post p "
          +
