...
public class UserServiceImpl implements UserService {
  ...
  public UserServiceImpl(UserRepository userRepository, BCryptPasswordEncoder passwordEncoder,
      CaptchaService captchaService, ImageService imageService,
  ...
  public ProfileEditResponse edit(ProfileEditRequest request, User user,
    ...
    ProfileEditResponse resp = new ProfileEditResponse();
    ...
    if (email != null) {
      if (findByEmail(email) == null || email.equalsIgnoreCase(user.getEmail())) {
        ...
        CookieManager.getInstance().addCookie(response, token, (int) jwtTokenProvider.getCookieMaxAge()-1);
      } else {
        errors.setEmail(ERROR_EMAIL);
        isDataCorrect = false;
      }
